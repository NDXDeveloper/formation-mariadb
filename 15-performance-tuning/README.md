...
Le fil conducteur tient en une règle simple mais exigeante : **mesurer avant d'optimiser**. Modifier une variable de configuration « parce qu'on l'a lue sur un blog » est l'erreur la plus répandue et la plus coûteuse. Chaque réglage doit partir d'une observation chiffrée — issue de `EXPLAIN` / `EXPLAIN ANALYZE` (vus au chapitre 5), du slow query log ou de Performance Schema — et son effet doit être vérifié de la même manière. L'optimisation est un cycle : observer, formuler une hypothèse, agir sur un seul paramètre à la fois, mesurer de nouveau.
